...
for /f %%I in ('powershell -NoProfile -Command "$u = $env:HEALTH_URL; try { $r = Invoke-WebRequest -UseBasicParsing -TimeoutSec 2 $u; if ($r.StatusCode -ge 200) { ''1'' } else { ''0'' } } catch { ''0'' }"') do set "SERVER_ALREADY_RUNNING=%%I"
...
for /l %%N in (1,1,60) do (
  for /f %%I in ('powershell -NoProfile -Command "$u = $env:HEALTH_URL; try { $r = Invoke-WebRequest -UseBasicParsing -TimeoutSec 2 $u; if ($r.StatusCode -ge 200) { ''1'' } else { ''0'' } } catch { ''0'' }"') do set "READY=%%I"
