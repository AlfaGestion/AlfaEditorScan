...
function usesTemplateOverride(element) {
  if (element?.tipo === 'textoFijo') return true
  if (element?.tipo === 'logo' || element?.tipo === 'linea' || element?.tipo === 'rectangulo') return false
  return Boolean(typeof element?.text === 'string' && element.text.trim())
}

function mapTypeToSql(element) {
  if (usesTemplateOverride(element)) return 'texto'
...
function mapFieldToSql(elementOrType) {
  if (elementOrType && typeof elementOrType === 'object' && usesTemplateOverride(elementOrType)) return 'TextoFijo'
...
function getFixedText(element) {
  if (usesTemplateOverride(element)) return element.text
